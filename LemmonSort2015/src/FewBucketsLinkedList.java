...
public class FewBucketsLinkedList {
	...
	private static void sort(String[] toSort) {
		...
		LinkedList<Data>[] buckets = (LinkedList<Data>[]) new LinkedList[10];
		for (int i = 0; i < 10; i++) {
			buckets[i] = new LinkedList<Data>();
		}
		...
		int index = 0;
		for (int i = 9; i >= 0; i--) {
			LinkedList<Data> bucket = buckets[i];
			// using iterator for linked list
			for (Data item : bucket) {
				toSort[index++] = item.fullString;
			}
		}
	}
